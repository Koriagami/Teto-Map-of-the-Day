...
const AVATAR_BORDER_COLOR = 'rgb(178, 228, 153)';
...
const STAT_LINE_COLOR_LEFT = 'rgb(255, 98, 213)';
const STAT_LINE_COLOR_RIGHT = 'rgb(197, 218, 41)';
...
async function drawCardInternal(leftUser, rightUser, scores, statWinners = null, loserSide = null) {
  ...
  if (play1 && play2) {
    ...
    for (let i = 0; i < STAT_DEFS.length; i++) {
      ...
      if (stat.textOnly) {
        ...
        ctx.font = `${valueFontSize}px ${CARD_FONT_FAMILY_BOLD || CARD_FONT_FAMILY}`;
  ...
  if (loserSide === 'left' || loserSide === 'right') {
    ...
    if (overlayImage) {
      ...
      const decoY = avatarY - overlayImage.height + 100;
